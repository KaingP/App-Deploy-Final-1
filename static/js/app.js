...
function initTabs() {
    ...
    const titles = {
        ...
        'tab-optimizer': 'Tinh Chỉnh Tham Số, Tối Ưu Hóa & Ca Bán Ngoài (OR-Tools)',
...
function initMoreMenu() {
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && sheet.classList.contains('active')) setOpen(false);
    });
}
...
function openConfirmModal({ title, message, confirmBtnText = 'Xác Nhận Xóa', onConfirm }) {
    ...
    if (msgEl) {
        msgEl.innerHTML = esc(message || 'Bạn có chắc chắn muốn thực hiện hành động này?');
    }
    if (btnProceed) {
...
function initModals() {
    ...
    document.getElementById('btnCloseShiftModal')?.addEventListener('click', () => shiftModal.classList.remove('active'));
    document.getElementById('btnCancelShiftEdit')?.addEventListener('click', () => shiftModal.classList.remove('active'));
    ...
    document.getElementById('btnClosePreviewModal')?.addEventListener('click', () => previewModal.classList.remove('active'));

    // Product Modal
    const productModal = document.getElementById('productModal');
    document.getElementById('btnCloseProductModal')?.addEventListener('click', () => productModal?.classList.remove('active'));
    document.getElementById('btnCancelProductModal')?.addEventListener('click', () => productModal?.classList.remove('active'));
    ...
    document.querySelectorAll('.modal-backdrop').forEach(backdrop => {
        backdrop.addEventListener('click', (e) => {
            if (e.target === backdrop) {
                ...
                if (backdrop.id === 'confirmModal') currentConfirmCallback = null;
            }
        });
    });
}
...
function initEventListeners() {
    ...
    document.getElementById('memberStatsSearchInput')?.addEventListener('input', () => {
        renderMemberTable();
    });
    ...
    document.getElementById('optimizerForm')?.addEventListener('submit', (e) => {
        ...
        const enableNgoai = document.getElementById('chkMasterNgoai') ? document.getElementById('chkMasterNgoai').checked : true;
    ...
    document.getElementById('saleSelectProduct')?.addEventListener('change', updateSaleCalcTotal);
    document.getElementById('saleInputQty')?.addEventListener('input', updateSaleCalcTotal);
    ...
    document.getElementById('formAddCaNgoai')?.addEventListener('submit', async (e) => {
        ...
        const chinh = parseInt(document.getElementById('selectNgoaiChinh').value);
    ...
    document.querySelectorAll('#channelFilter .filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('#channelFilter .filter-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            renderDutyBoard();
        });
    });

    document.getElementById('dayFilter')?.addEventListener('change', renderDutyBoard);
    document.getElementById('memberSearchInput')?.addEventListener('input', renderDutyBoard);

    // Upload & Sync Data
    document.getElementById('btnSyncGoogleSheet')?.addEventListener('click', handleSyncGoogleSheet);
    document.getElementById('btnUploadFile')?.addEventListener('click', handleUploadFile);

    // Shift Edit Save
    document.getElementById('btnSaveShiftEdit')?.addEventListener('click', handleSaveShiftEdit);

    // Incident Shift Select Change
    document.getElementById('incidentShiftSelect')?.addEventListener('change', handleIncidentShiftChange);
    document.getElementById('btnSubmitIncident')?.addEventListener('click', handleSubmitIncident);
    ...
    document.getElementById('btnCopyForGSheets')?.addEventListener('click', copyCurrentPreviewTable);
    ...
    document.getElementById('liveShiftSelect')?.addEventListener('change', (e) => {
        loadLiveShiftDetailsAndCandidates(e.target.value);
    });

    document.getElementById('btnClearLiveCart')?.addEventListener('click', () => {
        liveCart = {};
        renderLiveCart();
    });

    document.getElementById('btnCheckoutLivePOS')?.addEventListener('click', handleCheckoutLivePOS);
    ...
    document.getElementById('btnPOSQuickAdd')?.addEventListener('click', () => {
        ...
        if (!prodId) {
            alert('Vui lòng chọn 1 sản phẩm từ danh sách để thêm vào giỏ hàng!');
            return;
        }
        addToLiveCart(prodId);
    });

    document.getElementById('btnToggleLiveHeader')?.addEventListener('click', () => {
    ...
    document.getElementById('btnLiveOpenIncidentModal')?.addEventListener('click', () => {
    ...
    document.getElementById('btnUploadInventoryExcel')?.addEventListener('click', () => {
        document.getElementById('inputInventoryExcelFile')?.click();
    });
    document.getElementById('inputInventoryExcelFile')?.addEventListener('change', handleUploadInventoryExcel);

    // Export Contingency Incident Excel
    document.getElementById('btnExportIncidentExcel')?.addEventListener('click', () => {
    ...
    document.getElementById('btnExportKpiReport')?.addEventListener('click', () => {
        exportKpiReportCSV();
    });
...
function updateCaNgoaiPanelState(enabled) {
    ...
    if (enabled) {
        ...
        document.querySelectorAll('#formAddCaNgoai input, #formAddCaNgoai select, #formAddCaNgoai button, #btnClearAllCaNgoai').forEach(el => {
            el.disabled = false;
        });
    } else {
...
const solveUI = {
    ...
    step(n, state) {
        ...
        const icon = state === 'done' ? 'fa-solid fa-check'
            : state === 'busy' ? 'fa-solid fa-circle-notch fa-spin'
    ...
    open() {
        ...
        this.timer = setInterval(() => {
            if (out) out.textContent = Math.round((Date.now() - this.start) / 1000);
        }, 500);
    },
    ...
    seconds() { return Math.round((Date.now() - this.start) / 1000); }
};
...
async function runOptimizerWithParams(params) {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Đang giải bài toán tối ưu với OR-Tools...';
    }
    if (quickBtn) quickBtn.disabled = true;
    solveUI.open();

    try {
        const isNgoaiEnabled = document.getElementById('chkMasterNgoai') ? document.getElementById('chkMasterNgoai').checked : true;
        const payload = {
            ...params,
        ...
        if (data.success) {
            ...
            alert('Không giải được bài toán: ' + (data.message || 'lỗi không rõ'));
        }
    } catch (e) {
        alert('Mất kết nối tới server sau ' + solveUI.seconds() + ' giây: ' + e.message);
    } finally {
        solveUI.close();
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-solid fa-play"></i> Thực Thi Thuật Toán Phân Ca Tối Ưu (Google OR-Tools CP-SAT)';
        }
...
const BOARD_DAYS = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'Chủ Nhật'];
const DAY_ABBR = {
    'Thứ 2': 'T2', 'Thứ 3': 'T3', 'Thứ 4': 'T4', 'Thứ 5': 'T5',
    'Thứ 6': 'T6', 'Thứ 7': 'T7', 'Chủ Nhật': 'CN'
};
const BOARD_SLOTS = ['7h - 9h', '9h - 11h', '11h - 13h', '13h - 15h', '15h - 17h'];
...
function getFilteredShifts() {
    const channelFilter = document.querySelector('#channelFilter .filter-btn.active')?.getAttribute('data-filter') || 'all';
    ...
    return (globalScheduleData?.assigned_shifts || []).filter(s => {
        ...
        if (searchQuery) {
            const hit = s.shift_id.toLowerCase().includes(searchQuery)
                || s.location.toLowerCase().includes(searchQuery)
                || s.assigned_members.some(m =>
...
function renderDutyBoard() {
    ...
    filtered.forEach(s => {
        ...
        if (s.type === 'Phong' && s.date && !dateOfDay[s.day]) dateOfDay[s.day] = s.date;
    });

    const rows = BOARD_SLOTS.filter(sl => BOARD_DAYS.some(d => buckets[sl + '|' + d]));
...
function getMemberShiftCounts() {
    const counts = {};
    if (!globalScheduleData || !globalScheduleData.assigned_shifts) return counts;
...
function getMaxShiftsLimit() {
    if (globalScheduleData && globalScheduleData.config && globalScheduleData.config.max_shifts_per_member) {
...
function renderAlertStrip(filtered, shortShifts, totalGap) {
    ...
    const overlimitMembers = Object.keys(shiftCounts).filter(name => shiftCounts[name] > maxShifts);
    ...
    if (totalGap === 0) {
        ...
        const names = shortShifts.slice(0, 4)
            .map(s => `${esc(DAY_ABBR[s.day] || s.day)} ${esc(s.slot)}`).join(' · ');
        const more = shortShifts.length > 4 ? ` +${shortShifts.length - 4} ca nữa` : '';
...
function renderShiftSlip(s) {
    ...
    for (let i = 0; i < gap; i++) lines += `<span class="assign-line line-blank"></span>`;
...
function renderAgenda(filtered, shortShifts) {
    const daysPresent = BOARD_DAYS.filter(d => filtered.some(s => s.day === d));
    ...
    const list = ofDay.map(s => {
        const gap = shiftGap(s);
        const members = s.assigned_members || [];
        const chinh = members.filter(m => m.role === 'Chính');
        const dp = members.filter(m => m.role !== 'Chính');

        let people = chinh.concat(dp).map(m => {
            const isLeader = s.shift_leader === m.name;
            const isDp = m.role !== 'Chính';
            const totalCount = shiftCounts[m.name] || 0;
            const isOver = totalCount > maxShifts;
...
window.openShiftEditModal = function(shiftId) {
    if (!globalScheduleData) return;
    const s = globalScheduleData.assigned_shifts.find(item => item.shift_id === shiftId);
    if (!s) return;

    currentEditingShift = s;
    const modal = document.getElementById('shiftEditModal');
    document.getElementById('modalShiftTitle').innerHTML = `<i class="fa-solid fa-pen-to-square"></i> Chi Tiết & Chỉnh Sửa Ca ${esc(s.shift_id)} (${esc(s.day)} - ${esc(s.slot)})`;
    ...
    const shiftLocation = s.location || (s.type === 'Ngoai' ? 'Điểm Bán Ngoài Ca' : 'Phòng Thanh Niên');
    ...
    s.assigned_members.forEach((m, idx) => {
        ...
        const defaultRole = isDp 
            ...
            : (idx === 0 ? '💵 Thu ngân / Nhập sheet' : (idx === 1 ? '📦 Kiểm kê hàng' : '🛵 Phục vụ / Giao hàng'));
            ...
            <div class="member-edit-row ${isDp ? 'is-dp-row' : ''}">
                ...
                <div class="mer-controls">
                    <div class="mer-ctrl-item">
                        ...
                        <select class="custom-select member-role-select" data-member-id="${m.member_id}" onchange="handleRoleChangeInModal(this, '${m.member_id}')">
                    ...
                    <div class="mer-ctrl-item">
                        ...
                        <select class="custom-select member-position-select" data-member-id="${m.member_id}" id="pos_sel_${m.member_id}">
    ...
    const bodyHtml = `
        ...
        <div class="form-group mb-16">
            ...
            <select id="modalSelectLeader" class="custom-select">${leaderOptions}</select>
        </div>
...
window.handleRoleChangeInModal = function(roleSelect, memberId) {
    ...
    } else if (roleSelect.value === 'Chính' && posSelect && posSelect.value === '⚡ Dự bị tiếp ứng & Hỗ trợ') {
...
async function handleSaveShiftEdit() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            const idx = globalScheduleData.assigned_shifts.findIndex(s => s.shift_id === currentEditingShift.shift_id);
            if (idx !== -1) globalScheduleData.assigned_shifts[idx] = data.shift;

            renderDutyBoard();
            if (msg) {
                msg.className = 'swap-msg success';
                msg.textContent = '✓ Đã cập nhật Trưởng ca và phân công nhiệm vụ thành công!';
            }
            setTimeout(() => {
...
function renderHeatmapTable(heatmapData) {
    ...
    matrix.forEach(row => {
        ...
        row.slots.forEach(cell => {
            const memberNames = cell.free_members.slice(0, 10).map(m => m.name).join(', ') + (cell.free_members.length > 10 ? '...' : '');
            tableHtml += `
...
function renderKPIs() {
    ...
    document.getElementById('kpiTotalMembers').textContent = summary.total_members || 50;
    document.getElementById('kpiTotalShifts').textContent = summary.total_active_shifts || 70;
    document.getElementById('kpiAvgShifts').textContent = (summary.avg_shifts_per_member || 3.9).toFixed(1);
    document.getElementById('kpiFairness').textContent = `${audit.fairness_metrics?.fairness_score || 97}%`;
}
...
function renderMemberTable() {
    ...
    stats.forEach(m => {
        ...
        if (query) {
            ...
            if (!nameMatch && !deptMatch && !phoneMatch && !idMatch && !jobMatch) {
...
async function handleIncidentShiftChange() {
    ...
    if (!shiftId || !globalScheduleData) {
        if (absentSel) absentSel.innerHTML = '<option value="">-- Chọn thành viên --</option>';
        if (repSel) repSel.innerHTML = '<option value="">-- Chọn nhân sự thay thế --</option>';
        return;
    }
    ...
    let absentOpts = '<option value="">-- Chọn người vắng / đi trễ / đổi ca --</option>';
    s.assigned_members.forEach(m => {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            let repOpts = '<option value="">-- Chọn nhân sự thay thế --</option>';
            data.candidates.forEach(c => {
                const badge = c.priority_label || (c.is_standby ? '⚡ [Đội ứng biến]' : '✓ Rảnh');
                repOpts += `<option value="${c.member_id}">${badge} ${c.name} - ${c.department} (SĐT: ${c.phone})</option>`;
            });
            if (repSel) repSel.innerHTML = repOpts;
            if (hint) hint.textContent = `Tìm thấy ${data.total} ứng viên khả dụng (Ưu tiên theo Đội ứng biến & Lịch đã đăng ký rảnh)`;
        }
    } catch (e) {
...
async function handleSubmitIncident() {
    ...
    try {
        const res = await fetch('/api/contingency/log-incident', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                shift_id: shiftId,
...
function renderIncidentLogs(incidents) {
    ...
    if (!incidents || !incidents.length) {
        if (container) container.innerHTML = `<div class="empty-note">Chưa có sự cố điểm danh nào được ghi nhận.</div>`;
        if (tbody) tbody.innerHTML = `<tr><td colspan="8" class="table-empty">Chưa có dữ liệu đi trễ hoặc vắng mặt nào.</td></tr>`;
    ...
    incidents.forEach(inc => {
        ...
        if (inc.status_type === 'Vắng đột xuất' || inc.status_type === 'Xin nghỉ trước') absentCount++;
        if (inc.replacement_member && inc.replacement_member !== 'Không thay thế') replacedCount++;
...
async function handleSyncGoogleSheet() {
    ...
    try {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                document.getElementById('uploadDataModal').classList.remove('active');
                loadCurrentSchedule();
            }, 1500);
        } else {
            if (msg) {
...
function renderPreviewSheet(sheetKey) {
    ...
    if (!container || !globalPreviewData || !globalPreviewData[sheetKey]) return;
...
function copyCurrentPreviewTable() {
    if (!globalPreviewData || !globalPreviewData[currentPreviewSheetKey]) return;
    ...
    navigator.clipboard.writeText(tsv).then(() => {
        if (btn) {
            ...
            btn.innerHTML = '<i class="fa-solid fa-check"></i> Đã sao chép vào Clipboard!';
...
function renderInventoryKPIs(kpis) {
    ...
    if (stockEl) stockEl.textContent = `${(kpis.total_stock || 0).toLocaleString('vi-VN')} SP`;
    if (soldEl) soldEl.textContent = `${(kpis.total_sold || 0).toLocaleString('vi-VN')} SP`;
...
function renderInventoryTable(products) {
    ...
    let html = '';
    products.forEach(p => {
        const currentStock = p.current_stock !== undefined ? p.current_stock : (p.initial_stock - p.sold_count);
        let badgeClass = 'high';
        if (currentStock <= 0) {
            badgeClass = 'empty';
...
function renderSalesLogsTable(logs) {
    ...
    logs.forEach(l => {
        ...
        const refundBadge = isRefunded ? `<span class="status-badge badge-danger" style="background-color: rgba(239, 68, 68, 0.15); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.3); margin-left: 6px; font-size: 10px; padding: 1px 4px;">Đã hủy</span>` : '';
        const rowStyle = isRefunded ? 'style="opacity: 0.6; text-decoration: line-through;"' : '';
        const actionBtn = isRefunded ? `<span style="color: var(--ink-dim); font-size: 12px;"><i class="fa-solid fa-ban"></i> Đã hủy</span>` : `
...
function populateSaleProductOptions(products) {
    ...
    (products || []).forEach(p => {
        const cur = p.current_stock !== undefined ? p.current_stock : (p.initial_stock - p.sold_count);
...
function openProductModalForAdd() {
    ...
    document.getElementById('productModalTitle').innerHTML = `<i class="fa-solid fa-box-open"></i> Thêm Sản Phẩm Mới Vào Kho`;
...
window.openEditProductModal = function(productId) {
    ...
    const p = globalInventoryData.products.find(item => item.id === productId);
    if (!p) return;

    const modal = document.getElementById('productModal');
    if (!modal) return;

    document.getElementById('productModalTitle').innerHTML = `<i class="fa-solid fa-pen-to-square"></i> Cập Nhật Sản Phẩm ${p.id}`;
...
async function handleSaveProduct(e) {
    ...
    const price = parseInt(document.getElementById('inputProdPrice').value) || 0;
    const initial_stock = parseInt(document.getElementById('inputProdInitial').value) || 0;
    const sold_count = parseInt(document.getElementById('inputProdSold').value) || 0;
...
async function handleSaveSale(e) {
    ...
    const channel = document.getElementById('saleSelectChannel')?.value || 'Phòng Thanh Niên';
    const seller = document.getElementById('saleInputSeller')?.value || 'Ban Quản Trị';
...
function populateLiveShiftDropdown() {
    ...
    if (!select || !globalScheduleData || !globalScheduleData.assigned_shifts) return;
    ...
    shifts.forEach((s, idx) => {
        ...
        const loc = s.location || (s.type === 'Ngoai' ? 'Điểm Bán Ngoài' : 'Phòng Thanh Niên');
        const leader = s.shift_leader ? ` (Trưởng ca: ${s.shift_leader})` : '';
        html += `<option value="${s.shift_id}" ${isClosest ? 'selected' : ''}>
            ${badge} Ca ${s.shift_id} - ${s.day} (${s.slot || s.start_time}) - ${loc} [${s.assigned_count}/${s.required_count} TV]${leader}
        </option>`;
    });
...
async function loadLiveShiftDetailsAndCandidates(shiftId) {
    ...
    try {
        const res = await fetch(`/api/contingency/suggest?shift_id=${shiftId}`).then(r => r.json());
        if (res.success) {
    ...
    if (globalScheduleData && globalScheduleData.assigned_shifts) {
        ...
        if (shift) {
            ...
            (shift.assigned_members || []).forEach(m => {
                const lastLog = incidents.find(l => l.shift_id === shiftId && (l.absent_member === m.name || l.absent_member_id === m.member_id));
                localAttendanceState[m.member_id] = {
                    status: lastLog ? lastLog.status_type : null,
                    replacementId: lastLog ? (lastLog.replacement_member_id || '') : '',
                    isModified: false
                };
            });
        }
...
function renderLiveShiftDetails(shiftId) {
    ...
    if (subEl) {
        ...
        subEl.innerHTML = `Đang chọn ca: <strong style="color:var(--goldleaf);">${esc(shift.shift_id)}</strong> (${esc(shift.day)} ${esc(shift.slot)}) &bull; Địa điểm: <strong>${esc(loc)}</strong>`;
    }
    ...
    if (memberListEl) {
        ...
        (shift.assigned_members || []).forEach(m => {
            ...
            const isDp = m.role !== 'Chính';
            const task = m.position_role || (isDp ? '⚡ Dự bị tiếp ứng' : '🛵 Phục vụ / Bán hàng');

            // Get status and replacement from local state
            const localState = localAttendanceState[m.member_id] || { status: null, replacementId: '', isModified: false };
            ...
            let statusBadge = '<span class="tag" style="background:rgba(255,255,255,0.06); color:var(--ink-dim); border:1px solid rgba(255,255,255,0.1); padding:3px 8px;"><i class="fa-regular fa-circle"></i> Chưa điểm danh</span>';

            if (currentStatus === 'Có mặt') {
                cardBg = 'rgba(16,185,129,0.08)';
                cardBorder = '1px solid rgba(16,185,129,0.25)';
                cardLeftBorder = '4px solid #10B981';
                statusBadge = '<span class="tag" style="background:rgba(16,185,129,0.25); color:#34D399; border:1px solid #059669; padding:3px 8px; font-weight:700;"><i class="fa-solid fa-circle-check"></i> Có mặt đúng giờ</span>';
            } else if (currentStatus === 'Đi trễ') {
                cardBg = 'rgba(245,158,11,0.08)';
                cardBorder = '1px solid rgba(245,158,11,0.25)';
                cardLeftBorder = '4px solid #F59E0B';
                statusBadge = '<span class="tag" style="background:rgba(245,158,11,0.25); color:#FBBF24; border:1px solid #D97706; padding:3px 8px; font-weight:700;"><i class="fa-solid fa-clock-rotate-left"></i> Báo đi trễ</span>';
            } else if (currentStatus === 'Vắng đột xuất' || currentStatus === 'Xin nghỉ trước' || currentStatus === 'Vắng mặt') {
                cardBg = 'rgba(239,68,68,0.08)';
                cardBorder = '1px solid rgba(239,68,68,0.25)';
                cardLeftBorder = '4px solid #EF4444';
                statusBadge = '<span class="tag" style="background:rgba(239,68,68,0.25); color:#FCA5A5; border:1px solid #DC2626; padding:3px 8px; font-weight:700;"><i class="fa-solid fa-triangle-exclamation"></i> Vắng mặt khẩn cấp</span>';
            }

            // Build replacement dropdown HTML if marked as absent
            let replacementHtml = '';
            const isAbsent = (currentStatus === 'Vắng đột xuất' || currentStatus === 'Xin nghỉ trước' || currentStatus === 'Vắng mặt');
            if (isAbsent) {
                ...
                (currentShiftCandidates || []).forEach(cand => {
                    const selectedAttr = localState.replacementId === cand.member_id ? 'selected' : '';
    ...
    if (sellerSel) {
        ...
        sellerSel.innerHTML = sellerOpts || '<option value="Ban Quản Trị">Ban Quản Trị</option>';
    }
...
window.setLocalAttendanceStatus = function(memberId, status) {
    if (!localAttendanceState[memberId]) {
        localAttendanceState[memberId] = { status: null, replacementId: '', isModified: false };
    }
    ...
    if (status !== 'Vắng đột xuất' && status !== 'Xin nghỉ trước' && status !== 'Vắng mặt') {
...
window.submitLiveAttendanceBatch = async function() {
    ...
    modifiedKeys.forEach(k => {
        ...
        const shift = globalScheduleData.assigned_shifts.find(s => s.shift_id === currentSelectedLiveShiftId);
        const member = shift?.assigned_members?.find(sm => sm.member_id === k) || { name: k };
        
        let repName = 'Không thay thế';
        if (mState.replacementId && mState.replacementId !== 'no_replacement') {
            const repObj = currentShiftCandidates.find(c => c.member_id === mState.replacementId) || globalScheduleData.member_stats?.find(ms => ms.member_id === mState.replacementId);
        ...
        onConfirm: async () => {
            try {
                ...
                for (const k of modifiedKeys) {
                    ...
                    if (!res.success) {
                        console.error('Lỗi khi ghi nhận thành viên', k, res.message);
                    }
                }
                ...
                alert('✓ Đã cập nhật điểm danh & kích hoạt phương án backup thành công!');
            } catch (e) {
                console.error('Lỗi kết nối điểm danh:', e);
                alert('Có lỗi xảy ra trong quá trình ghi nhận điểm danh: ' + e.message);
            } finally {
                const btn = document.getElementById('btnConfirmLiveAttendance');
                if (btn) {
                    btn.disabled = false;
...
function renderLivePOSProductGrid() {
    ...
    if (quickSel) {
        ...
        products.forEach(p => {
            const stock = p.current_stock !== undefined ? p.current_stock : Math.max(0, (p.initial_stock || 0) - (p.sold_count || 0));
    ...
    if (!products.length) {
        grid.innerHTML = '<div style="grid-column: 1/-1; color: var(--ink-dim); font-size: 13px; padding: 12px; text-align: center;">Kho sản phẩm chưa có dữ liệu. Hãy thêm sản phẩm ở tab Kho Hàng.</div>';
        return;
    }
    ...
    products.forEach(p => {
        ...
        const isOutOfStock = stock <= 0;
        html += `
            <div style="background: rgba(255,255,255,0.04); border: 1px solid var(--border-color); border-radius: 8px; padding: 10px; text-align: center; display: flex; flex-direction: column; justify-content: space-between; transition: all 0.2s ease;">
                <div>
                    <strong style="font-size: 13px; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #FFF;" title="${esc(p.name)}">${esc(p.name)}</strong>
                    <span style="font-size: 13px; color: var(--goldleaf); font-weight: 700; display: block; margin: 4px 0;">${formatVND(p.price)}</span>
                    <span style="font-size: 11px; color: ${isOutOfStock ? '#F87171' : '#34D399'}; display: block; margin-bottom: 8px; font-weight: 500;">Tồn: ${stock} ${p.unit}</span>
                </div>
                <button type="button" class="btn-action-sm btn-action-edit" onclick="addToLiveCart('${p.id}')" ${isOutOfStock ? 'disabled style="opacity:0.4; cursor:not-allowed;"' : ''} style="width: 100%; font-size: 12px; padding: 5px 0; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 4px; cursor: pointer;">
...
window.addToLiveCart = function(prodId) {
    ...
    if (!liveCart[prodId]) {
        ...
        if (liveCart[prodId].quantity < stock) {
            ...
            alert(`Đã đạt giới hạn tồn kho tối đa (${stock} ${p.unit}) của sản phẩm ${p.name}`);
        }
    }
    renderLiveCart();
};
...
window.updateLiveCartQty = function(prodId, delta) {
    ...
    if (delta > 0 && newQty > stock) {
        alert(`Không thể tăng quá số lượng tồn kho hiện có (${stock} ${p.unit})!`);
        return;
    }
...
async function handleCheckoutLivePOS() {
    ...
    const seller = document.getElementById('livePOSSelectSeller')?.value || 'Thu ngân ca';
    ...
    try {
        ...
        if (msg) {
            msg.className = 'swap-msg success';
            msg.textContent = '✓ Thanh toán thành công! Doanh thu đã được cộng vào ca này.';
        }
...
function renderLiveShiftSalesTable(shiftId) {
    ...
    const shiftSales = allSales.filter(l => (l.channel || '').includes(shiftId));

    const totalRev = shiftSales.reduce((acc, l) => acc + (l.refunded ? 0 : (l.total_amount || 0)), 0);
    if (summaryEl) summaryEl.textContent = `${formatVND(totalRev)} ca ${shiftId}`;
    ...
    shiftSales.forEach(l => {
        const isRefunded = l.refunded === true;
        const rowStyle = isRefunded ? 'style="opacity: 0.5; text-decoration: line-through;"' : '';
        const refundStatus = isRefunded ? `<div style="color: #ef4444; font-size: 10px; font-weight: 600; margin-top:2px;"><i class="fa-solid fa-ban"></i> Đã hủy</div>` : '';
        const actionBtn = isRefunded ? `<span style="color: var(--ink-dim); font-size: 11px;"><i class="fa-solid fa-ban"></i> Đã hủy</span>` : `
...
async function refundTransaction(txId) {
    if (!confirm(`Bạn có chắc chắn muốn HỦY & HOÀN TÁC giao dịch ${txId} này?\nSố lượng sản phẩm tương ứng sẽ được hoàn lại vào kho hàng.`)) {
        return;
    }
...
function openEditMemberModal(memberId) {
    ...
    days.forEach(day => {
        ...
        slots.forEach(slot => {
            const key = `${day}|${slot}`;
            const isFree = member.availability && member.availability[key] === true;
            const isCommit = member.committed_slots && member.committed_slots[key] === true;
...
async function saveMemberData() {
    ...
    try {
        ...
        if (data.success) {
            ...
            const resMembers = await fetch('/api/members').then(r => r.json());
            if (resMembers.success) {
                globalMembers = resMembers.members;
            }

            // Reload schedule to update KPI & members stats
            const resSched = await fetch('/api/schedule/current').then(r => r.json());
            if (resSched.success) {
                globalScheduleData = resSched.result;
            }
...
async function loadKpiData() {
    ...
    try {
        ...
        if (res.success) {
            ...
            if (tableBody) tableBody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--cinnabar);">Lỗi: ${res.message}</td></tr>`;
        }
    } catch (err) {
        console.error('Lỗi tải dữ liệu KPI:', err);
        if (tableBody) tableBody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--cinnabar);">Không kết nối được server để tải KPI</td></tr>`;
    }
}
...
function renderKpiStats() {
    ...
    const onTimeCount = globalKpiAttendance.filter(l => l.status === 'Đúng giờ').length;
    const lateCount = globalKpiAttendance.filter(l => l.status === 'Đi trễ').length;
    const absentCount = globalKpiAttendance.filter(l => l.status === 'Nghỉ không phép').length;
...
function renderKpiAttendance() {
    ...
    filtered.forEach(log => {
        ...
        else if (log.status === 'Ứng biến standby') selectClass = 'select-purple';
        ...
        html += `
            <tr style="transition: background-color 0.2s;">
                ...
                <td>${roleTag}</td>
                <td>
                    <select class="custom-select ${selectClass}" 
...
async function updateKpiStatus(shiftId, memberId, newStatus, selectElement) {
    ...
    try {
        const res = await fetch('/api/kpi/attendance', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ shift_id: shiftId, member_id: memberId, status: newStatus })
        }).then(r => r.json());
        
        if (res.success) {
            // Update local memory
            const log = globalKpiAttendance.find(l => l.shift_id === shiftId && l.member_id === memberId);
            if (log) {
            ...
            if (selectElement) {
                ...
                else if (newStatus === 'Nghỉ có phép') selectClass = 'select-blue';
                else if (newStatus === 'Nghỉ không phép') selectClass = 'select-red';
                else if (newStatus === 'Ứng biến standby') selectClass = 'select-purple';
...
function renderKpiLeaderboard() {
    ...
    allMembers.forEach(m => {
        ...
        // Total scheduled ca
        const totalShifts = logs.filter(l => l.status !== 'Ứng biến standby').length;
        
        // Compute score percentage
        let score = 100;
        const expectedPoints = totalShifts * 10;
        if (expectedPoints > 0 || standby > 0) {
            const earnedPoints = (onTime * 10) + (late * 5) + (excused * 3) - (unexcused * 15) + (standby * 15);
            const expected = expectedPoints > 0 ? expectedPoints : 10;
            score = Math.max(0, Math.min(150, Math.round((earnedPoints / expected) * 100)));
        }
    ...
    filtered.forEach((item, index) => {
        ...
        if (rank === 1) rankBadge = `🏆 <span style="font-weight:800; color: #d97706;">1</span>`;
        else if (rank === 2) rankBadge = `🥈 <span style="font-weight:800; color: #4b5563;">2</span>`;
        else if (rank === 3) rankBadge = `🥉 <span style="font-weight:800; color: #b45309;">3</span>`;
...
function exportKpiReportCSV() {
    ...
    const rows = [
        ...
        [],
        ['Hạng', 'Mã Thành Viên', 'Thành Viên', 'Ban Chuyên Môn', 'Tổng Ca Phân Công', 'Số Ca Đúng Giờ', 'Số Ca Đi Trễ', 'Nghỉ Có Phép', 'Vắng Không Phép', 'Số Ca Ứng Cứu Standby', 'Điểm Số KPI (%)', 'Xếp Loại Hiệu Suất']
    ];
    ...
    allMembers.forEach(m => {
        ...
        let onTime = 0, late = 0, excused = 0, unexcused = 0, standby = 0;
        logs.forEach(l => {
            if (l.status === 'Đúng giờ') onTime++;
            else if (l.status === 'Đi trễ') late++;
            else if (l.status === 'Nghỉ có phép') excused++;
    ...
    link.setAttribute("download", `Bao_Cao_KPI_Chuyen_Can_${new Date().toISOString().slice(0, 10)}.csv`);
